...
int main( int argc, char* argv[] ) {
  ...
  /**
    Linkbench: run a fixed number of queries
    * do this benchmark last, since it alters the graph database
   */
  if( directed ) {
    if( rank == 0 ) {
      fprintf( stderr, "Linkbench queries currently only support undirected edges.\n" );
    }
    MPI_Abort( MPI_COMM_WORLD, 1 );
  }

  uint32_t rcount = cli.rcount();
  benchmark_linkbench( db, vlabels, elabels, ptypes, nglobalverts, rcount );
