...
void benchmark_linkbench( GDI_Database db, GDI_Label* vlabels, GDI_Label* elabels, GDI_PropertyType* ptypes, uint64_t nglobalverts, uint32_t num_measurements ) {
  ...
  /**
    Facebook TAO probabilities from the Weaver paper (called read-mostly):
    99.8% read:
    - 59.4% get edges (used 59.3%)
    - 11.7% count edges
    - 28.9% get vertex (used 28.8%)
     0.2% write:
    - 80.0 % create edge (only support creation of edge, so used 0.2%)
    - 20.0 % delete edge
   */
  uint8_t linkbench_query_count = 7;
  uint16_t* linkbench_query_probability = malloc( linkbench_query_count * sizeof(uint16_t) );
  linkbench_query_probability[0] =  288; /* linkbench_get_*_vertex */
  linkbench_query_probability[1] =  288; /* linkbench_add_vertex */
  linkbench_query_probability[2] =  288; /* linkbench_delete_vertex */
  linkbench_query_probability[3] =  288; /* linkbench_update_vertex */
  linkbench_query_probability[4] =  405; /* linkbench_count_edges */
  linkbench_query_probability[5] =  998; /* linkbench_get_edges */
  linkbench_query_probability[6] = 1000; /* linkbench_add_edge: added probability from delete edge */

#ifndef THROUGHPUT
  LSB_Init( "gdi_oltp.rm.lat" /* project name */, 0 /* autoprofiling interval, deactivated */);

  MPI_Barrier( MPI_COMM_WORLD ); /* start all queries more or less at the same time */
#else
  LSB_Init( "gdi_oltp.rm.tp" /* project name */, 0 /* autoprofiling interval, deactivated */);
