...
void benchmark_linkbench( GDI_Database db, GDI_Label* vlabels, GDI_Label* elabels, GDI_PropertyType* ptypes, uint64_t nglobalverts, uint32_t num_measurements ) {
  ...
  /**
    75% read/25% write workload from the Weaver paper: don't specify
  ...
  linkbench_query_probability[0] =  217; /* linkbench_get_*_vertex */
  linkbench_query_probability[1] =  217; /* linkbench_add_vertex */
  linkbench_query_probability[2] =  217; /* linkbench_delete_vertex */
  linkbench_query_probability[3] =  217; /* linkbench_update_vertex */
  linkbench_query_probability[4] =  305; /* linkbench_count_edges */
  linkbench_query_probability[5] =  750; /* linkbench_get_edges */
  linkbench_query_probability[6] = 1000; /* linkbench_add_edge: added probability from delete edge */

#ifndef THROUGHPUT
  LSB_Init( "gdi_oltp.ri.lat" /* project name */, 0 /* autoprofiling interval, deactivated */);

  MPI_Barrier( MPI_COMM_WORLD ); /* start all queries more or less at the same time */
#else
  LSB_Init( "gdi_oltp.ri.tp" /* project name */, 0 /* autoprofiling interval, deactivated */);
