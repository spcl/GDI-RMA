...
int main( int argc, char* argv[] ) {
  ...
  /**
    run business intelligence benchmark:
    * number of runs is hardcoded to 10
   */
  if( directed ) {
    if( rank == 0 ) {
      fprintf( stderr, "Business intelligence query currently only supports undirected edges.\n" );
    }
    MPI_Abort( MPI_COMM_WORLD, 1 );
  }
  uint32_t rcount = 10;
  benchmark_bi( db, vlabels, elabels, ptypes, nglobalverts, rcount );
